...
test('02 User requires an existing tenant', async () => {
  await rejects(
    prisma.user.create({ data: { id: id(), tenantId: id(), email: 'missing@example.test' } }),
  );
});

test('03 same email is allowed across tenants', async () => {
  const a = await prisma.tenant.create({ data: { id: id() } });
  const b = await prisma.tenant.create({ data: { id: id() } });
...
test('04 same email is rejected within one tenant', async () => {
  ...
  await prisma.user.create({
    data: { id: id(), tenantId: tenant.id, email: 'duplicate@example.test' },
  });
  await rejects(
    prisma.user.create({
      data: { id: id(), tenantId: tenant.id, email: 'duplicate@example.test' },
...
test('05 identity link requires a tenant-qualified user', async () => {
  await rejects(
    prisma.externalIdentityLink.create({
      data: {
        id: id(),
        tenantId: id(),
        userId: id(),
...
test('06 cross-tenant identity link is rejected by the database', async () => {
  ...
  const user = await prisma.user.create({
    data: { id: id(), tenantId: a.id, email: 'a@example.test' },
  });
  await rejects(
    prisma.externalIdentityLink.create({
      data: {
...
test('07 same external identity is allowed across tenants', async () => {
  ...
  const ua = await prisma.user.create({
    data: { id: id(), tenantId: a.id, email: 'ia@example.test' },
  });
  const ub = await prisma.user.create({
    data: { id: id(), tenantId: b.id, email: 'ib@example.test' },
  });
...
test('08 duplicate external identity is rejected within a tenant', async () => {
  const t = await prisma.tenant.create({ data: { id: id() } });
  const u = await prisma.user.create({
    data: { id: id(), tenantId: t.id, email: 'i@example.test' },
  });
  const data = {
    tenantId: t.id,
    userId: u.id,
  ...
  await prisma.externalIdentityLink.create({ data: { id: id(), ...data } });
  await rejects(prisma.externalIdentityLink.create({ data: { id: id(), ...data } }));
});

test('09 deleting a tenant with users is rejected', async () => {
  const t = await prisma.tenant.create({ data: { id: id() } });
  await prisma.user.create({
    data: { id: id(), tenantId: t.id, email: 'delete-tenant@example.test' },
  });
...
test('10 deleting a user with identity links is rejected', async () => {
  const t = await prisma.tenant.create({ data: { id: id() } });
  const u = await prisma.user.create({
    data: { id: id(), tenantId: t.id, email: 'delete-user@example.test' },
  });
  await prisma.externalIdentityLink.create({
    data: {
      id: id(),
